...
def loadModel(model_name='mask_fill', model_version='1', url='127.0.0.1:8001',loading = "POLLING"):
...
def run_inference(triton_client, premise = '',model_name='zst', model_version='1' , top_k = 5):
...
try:
    ...
    def question_answer(premise,top_k):
        ...
        top_k_tokens,top_k_probs = run_inference(client,premise,MODEL_NAME,MODEL_VERSION,int(top_k))
    ...
    with gr.Blocks() as demo:
        ...
        premiseBox = gr.Textbox(placeholder="Text to mask...",label="Mask token: <mask>")
        topKbox = gr.Number(value=5,label= "Number of masks")

        comp_btn = gr.Button("Compute")

        output = gr.Textbox(label="Probabilities",placeholder="<Token> (<Probability of being true>)")

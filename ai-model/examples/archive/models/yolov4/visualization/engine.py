...
async def visualize(
    media: UploadFile = File(), outputs: str = Form()
) -> StreamingResponse:
    ...
    input_image = cv2.imencode(ext=".jpg", img=input_image)[
        1
    ]  # hard-code to export image as jpeg

    return StreamingResponse(
        content=io.BytesIO(input_image.tobytes()), media_type="image/jpeg"
    )
